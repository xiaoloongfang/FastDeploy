...
PointPillarsPostprocessor::PointPillarsPostprocessor() {}

bool PointPillarsPostprocessor::Run(const std::vector<FDTensor>& tensors,
                             std::vector<PerceptionResult>* results) {
  // TODO: PointPillar Postprocessor need to be implemented. FDTensor should be transfor to PerceptionResult.
    results->resize(1);
  (*results)[0].Clear();
